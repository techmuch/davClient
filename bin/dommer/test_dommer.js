...
function test_parseXML_comments() {
    var dom = new dommer.DOM();
    ...
    testing.assertEquals(doc.childNodes.length, 1);
    testing.assertEquals(doc.documentElement.childNodes.length, 1);
    testing.assertEquals(doc.documentElement.childNodes[0].nodeType, 8);
    testing.assertEquals(doc.documentElement.childNodes[0].nodeValue,
                         ' <bar>baz</bar> ');
};

function test_getElementsByTagNameOnDocument() {
    var dom = new dommer.DOM();
    var doc = dom.parseXML('<foo><bar /></foo>');
    var els = doc.getElementsByTagName('bar');
    testing.assertEquals(els.length, 1);
};

function test_getElementById() {
    var dom = new dommer.DOM();
    var doc = dom.parseXML('<foo><bar id="bar" /></foo>');
    var el = doc.getElementById('bar');
    testing.assertEquals(el.getAttribute('id'), 'bar');
    var el = doc.getElementById('baz');
    testing.assertEquals(el, undefined);

    var doc = dom.parseXML('<foo><bar xml:id="bar" /></foo>');
    testing.debug('...');
    var el = doc.getElementById('bar', 'xml:id');
    testing.assertEquals(el.getAttribute('xml:id'), 'bar');
};

function test_firstChild() {
    var dom = new dommer.DOM();
    var doc = dom.parseXML('<foo><bar /></foo>');
    var el = doc.documentElement;
    testing.assertEquals(el.firstChild.nodeName, 'bar');
    el.appendChild(doc.createElement('baz'));
    testing.assertEquals(el.firstChild.nodeName, 'bar');
    el.removeChild(el.firstChild);
    testing.assertEquals(el.firstChild.nodeName, 'baz');
    el.insertBefore(doc.createElement('qux'), el.firstChild);
    testing.assertEquals(el.firstChild.nodeName, 'qux');
};

function test_ownerDocument() {
    var dom = new dommer.DOM();
    var doc = dom.parseXML('<foo><bar /></foo>');
    testing.assertEquals(doc.documentElement.ownerDocument, doc);
    testing.assertEquals(doc.getElementsByTagName('foo')[0].ownerDocument,
                         doc);
    testing.assertEquals(doc.getElementsByTagName('bar')[0].ownerDocument,
                         doc);
    testing.assertEquals(doc.documentElement.getElementsByTagName('bar')[0]
                            .ownerDocument, doc);
    testing.assertEquals(doc.documentElement.childNodes[0].ownerDocument, doc);
    var baz = doc.createElement('baz');
    testing.assertEquals(baz.ownerDocument, doc);
    doc.documentElement.appendChild(baz);
    testing.assertEquals(doc.getElementsByTagName('baz')[0].ownerDocument,
                         doc);
    var otherdoc = dom.parseXML('<qux />');
    testing.assertEquals(otherdoc.documentElement.ownerDocument, otherdoc);
    var qux = otherdoc.documentElement;
    qux = doc.importNode(qux);
    testing.assertEquals(qux.ownerDocument, doc);
    doc.documentElement.appendChild(qux);
    testing.assertEquals(qux.ownerDocument, doc);
    testing.assertEquals(doc.getElementsByTagName('qux')[0].ownerDocument,
                         doc);
};
